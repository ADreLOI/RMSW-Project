...
```text
II2202-proposal.tex          Overleaf main document (keep this setting)
proposal/
  ...
  references-section.tex    Reference-list guidance/placeholder
  references.bib            Project bibliography, initially empty
  assets/                   Figures used in the report
  appendices/               Optional appendices
  styles/                   Preserved bibliography style
  legacy/                   Unused old helpers and sample references
docs/                       Drafting worksheet
simulation/                 Future simulator, configurations and tests
data/                       Data provenance; no private or bulk raw data
```
...
In Overleaf, keep **Main document** set to `II2202-proposal.tex` and use **LuaLaTeX**. Compile from the repository root locally too, if a TeX distribution is installed.
...
Start with [the Section 2 worksheet](docs/section-2-worksheet.md). It contains feedback and author decision prompts, not submission prose. Keep final scientific text student-authored and record AI use as Canvas requires.

The reference placeholder is intentionally still present. When citations are added, replace it with the chosen reference system. If using the preserved BibTeX style, paths are `proposal/styles/myIEEEtran` and `proposal/references`. Do not cite the unrelated entries in `proposal/legacy/template-references.bib`.
